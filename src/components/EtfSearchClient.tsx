...
export default function EtfSearchClient({ initialEtfs }: EtfSearchClientProps) {
  ...
  const renderSortIcon = (column: keyof EtfWithPrice) => {
    if (sortColumn !== column) return <span className="ml-1.5 text-white/20 text-[10px]">↕</span>;
    return sortDirection === 'asc' ? (
      <span className="ml-1.5 text-red-accent text-[10px]">▲</span>
    ) : (
      <span className="ml-1.5 text-sky-primary text-[10px]">▼</span>
    );
  };

  // 수익률 글자색 클래스 결정
  const getYieldColor = (val: number | null) => {
    if (val === null || val === undefined || val === 0) return 'text-white/50';
    return val > 0 ? 'text-white font-bold' : 'text-red-accent font-bold';
  };
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between px-2">
                <span className="text-xs text-white/60 font-semibold select-none">
                  검색 결과: <strong className="text-white text-sm font-extrabold">{sortedList.length}</strong>개 종목
                </span>
              </div>

              {/* 검색 결과 표 */}
              {sortedList.length > 0 ? (
                <div className="overflow-x-auto rounded-2xl border border-white/5 bg-navy/40 shadow-xl">
                  <table className="w-full text-left border-collapse text-sm">
                    <thead>
                      <tr className="border-b border-white/10 bg-white/5 text-white/80 font-bold text-xs sm:text-sm uppercase tracking-wider select-none">
                        <th
                          onClick={() => handleSort('ticker')}
                          className="py-4.5 px-5 sm:px-8 cursor-pointer hover:bg-white/5 transition-colors"
                        >
                          티커 {renderSortIcon('ticker')}
                        </th>
                        <th
                          onClick={() => handleSort('name')}
                          className="py-4.5 px-4 cursor-pointer hover:bg-white/5 transition-colors"
                        >
                          이름 {renderSortIcon('name')}
                        </th>
                        <th
                          onClick={() => handleSort('close')}
                          className="py-4.5 px-4 text-right cursor-pointer hover:bg-white/5 transition-colors"
                        >
                    ...
                    <tbody className="divide-y divide-white/5 text-xs sm:text-sm text-white/90">
                      {sortedList.map((etf) => (
                        ...
                        >
                          <td className="py-4 px-5 sm:px-8 font-mono font-bold text-yellow-accent">
                            {etf.ticker}
                          </td>
                          <td className="py-4 px-4 font-semibold max-w-64 sm:max-w-85 truncate" title={etf.name}>
                            {etf.name}
                          </td>
                          <td className="py-4 px-4 text-right font-mono font-bold">
                            {etf.close !== null ? `${etf.close.toLocaleString()}원` : '-'}
                          </td>
                          <td className={`py-4 px-4 text-right font-mono ${getYieldColor(etf.yield_1w)}`}>
                            {formatYield(etf.yield_1w)}
                          </td>
                          <td className={`py-4 px-4 text-right font-mono ${getYieldColor(etf.yield_5w)}`}>
                            {formatYield(etf.yield_5w)}
                          </td>
                          <td className={`py-4 px-4 text-right font-mono ${getYieldColor(etf.yield_20w)}`}>
                            {formatYield(etf.yield_20w)}
                          </td>
                          <td className={`py-4 px-4 text-right font-mono ${getYieldColor(etf.yield_60w)}`}>
                            {formatYield(etf.yield_60w)}
                          </td>
                          <td className={`py-4 px-4 text-right font-mono ${getYieldColor(etf.yield_120w)}`}>
